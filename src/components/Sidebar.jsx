import React from "react";
import '../styles/Sidebar.css';

function Sidebar({lists, onAdd, onDelete, onSelect, onEdit}) {
    const set = (title) => {
        if (document.getElementById(title + '-input') && 
            document.getElementById(title + '-input').style.display === 'block') {
            document.getElementById(title + '-input').style.display = 'none';
            document.getElementById(title + '-label').style.display = 'block';
            document.getElementById(title + '-edit').style.display = 'block';
            document.getElementById(title + '-check').style.display = 'none';
        } else if (document.getElementById(title + '-input') && 
            document.getElementById(title + '-input').style.display === 'none') {
            document.getElementById(title + '-input').style.display = 'block';
            document.getElementById(title + '-label').style.display = 'none';
            document.getElementById(title + '-edit').style.display = 'none';
            document.getElementById(title + '-check').style.display = 'block';
            document.getElementById(title + '-input').value = title;
        }
    }
    const edit = (title) => {
        onEdit(title, document.getElementById(title + '-input').value);
    }
    const add = () => {
        onAdd(document.getElementById("task-lists").value);
        document.getElementById("task-lists").value = "";
    }
    const remove = (event) => {
        onDelete(event.currentTarget.id);
    }
    return (
        <aside>
            <ul>
                {lists?.map((list) => (
                    <li key={list.title} className="task-list-a">
                        <label id={list.title + '-label'} onClick={() => onSelect(list.title)}>{list.title}</label>
                        <input id={list.title + '-input'} style={{display: 'none'}} />
                        <div className="buttons">
                            <button id={list.title + '-edit'} onClick={() => set(list.title)}>
                                <svg xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 576 512"><path d="M402.6 83.2l90.2 90.2c3.8 3.8 3.8 10 0 13.8L274.4 405.6l-92.8 10.3c-12.4 1.4-22.9-9.1-21.5-21.5l10.3-92.8L388.8 83.2c3.8-3.8 10-3.8 13.8 0zm162-22.9l-48.8-48.8c-15.2-15.2-39.9-15.2-55.2 0l-35.4 35.4c-3.8 3.8-3.8 10 0 13.8l90.2 90.2c3.8 3.8 10 3.8 13.8 0l35.4-35.4c15.2-15.3 15.2-40 0-55.2zM384 346.2V448H64V128h229.8c3.2 0 6.2-1.3 8.5-3.5l40-40c7.6-7.6 2.2-20.5-8.5-20.5H48C21.5 64 0 85.5 0 112v352c0 26.5 21.5 48 48 48h352c26.5 0 48-21.5 48-48V306.2c0-10.7-12.9-16-20.5-8.5l-40 40c-2.2 2.3-3.5 5.3-3.5 8.5z"/></svg>
                            </button>
                            <button onClick={() => edit(list.title)} id={list.title + '-check'} style={{display: 'none'}} className="mark-button">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="#FFF" height="1em" viewBox="0 0 448 512">
                                    <path d="M438.6 105.4c12.5 12.5 12.5 32.8 0 45.3l-256 256c-12.5 12.5-32.8 12.5-45.3 0l-128-128c-12.5-12.5-12.5-32.8 0-45.3s32.8-12.5 45.3 0L160 338.7 393.4 105.4c12.5-12.5 32.8-12.5 45.3 0z"/>
                                </svg>
                            </button>
                            <button id={list.title} onClick={(e) => remove(e)}>X</button>
                        </div>
                    </li>
                ))}
            </ul>
            <div className="task-list-b">
                <input
                type="text"
                placeholder="Create a new list..."
                className="task-input"
                id="task-lists"
                />
                <button className="create-button" onClick={add}>
                +
                </button>
        </div>
        </aside>
    );
}

export default Sidebar;